...
int art_get_code(int animation, int weaponType, char* weaponCodePtr, char* animCodePtr)
{
    ...
    if (animation >= ANIM_TAKE_OUT && animation <= ANIM_FIRE_CONTINUOUS) {
        *animCodePtr = 'c' + (animation - ANIM_TAKE_OUT);
        ...
        *animCodePtr = 'a' + (animation - FIRST_SF_DEATH_ANIM);
        *weaponCodePtr = 'r';
        return 0;
    } else if (animation >= FIRST_KNOCKDOWN_AND_DEATH_ANIM) {
        *animCodePtr = 'a' + (animation - FIRST_KNOCKDOWN_AND_DEATH_ANIM);
        *weaponCodePtr = 'b';
        return 0;
